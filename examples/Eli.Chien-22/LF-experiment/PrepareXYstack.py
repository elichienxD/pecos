...
def main():
    ...
    data_dir= f"{word_dir}/dataset/{args.dataset}/normalized"
    ...
    X_bow_trn = smat.load_npz(f"{data_dir}/X_bow.trn.npz")
    Y_bow = smat.load_npz(f"{data_dir}/Y_bow.npz")
    ...
    smat.save_npz(f"{data_dir}/X_bow.all.npz",X_bow_all)
    del Y_bow, X_bow_trn, X_bow_all
    
    # Prepare text file (X_all.txt), note that the order should be X|L
    with open("{}/{}.txt".format(data_dir,split),'r') as f:
        X_lines = f.readlines()
