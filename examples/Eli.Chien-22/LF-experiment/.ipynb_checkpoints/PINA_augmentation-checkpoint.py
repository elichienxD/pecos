...
def main():
    ...
    params_path=f"{args.work_dir}/scripts/params/xtransformer/{args.model_name}.json"
    model_dir=f"{args.work_dir}/models/xtransformer/{args.dataset}/{args.model_name}/{args.feature_name}/XYstack"
    ...
    if args.L_option == "Lft_xrt":
        ...
        with open(f"{args.work_dir}/dataset/{args.dataset}/normalized/output-items.txt") as f:
            text = f.readlines()
        L_emb = xtf.encode(text, batch_size=512, batch_gen_workers=64)
        
        # Generate xrt dense embedding for instance text
        with open("{}/dataset/LF-dataset/{}/normalized/X.trn.txt".format(work_dir,dname)) as f:
            text = f.readlines()
        X_emb = xtf.encode(text, batch_size=512, batch_gen_workers=64)
        ...
        if args.feature_name in ['BoW']:
            ...
            X_all = smat.load_npz('{}/X.tfidf.all.npz'.format(feature_dir)).astype(np.float32)   
        X_all = sklearn.preprocessing.normalize(X_all,norm='l2')
        Y_all = smat.load_npz('{}/dataset/LF-dataset/{}/normalized/Y_all.npz'.format(work_dir,dname)).astype(np.float32)
        ...
        with open("{}/dataset/LF-dataset/{}/normalized/output-items.txt".format(work_dir,dname)) as f:
            text = f.readlines()
        L_emb = xtf.encode(text, batch_size=512, batch_gen_workers=64)
        with open("{}/dataset/LF-dataset/{}/normalized/X.trn.txt".format(work_dir,dname)) as f:
            text = f.readlines()
    ...
    with open(f"{args.work_dir}/dataset/{args.dataset}/normalized/X.trn.txt") as f:
        text = f.readlines()
    X_emb_trn = xtf.encode(text, batch_size=512, batch_gen_workers=64)

    with open(f"{args.work_dir}/dataset/{args.dataset}/normalized/X.tst.txt") as f:
        text = f.readlines()
    X_emb_tst = xtf.encode(text, batch_size=512, batch_gen_workers=64)
